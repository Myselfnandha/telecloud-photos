...
class MobileAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Look for small touch targets on interactive components
        if is_react_native:
            small_sizes = re.findall(r'(?:touchable|button|pressable).*?(?:width|height):\s*([0-3]\d)', content, re.IGNORECASE | re.DOTALL)
            for size in small_sizes:
                if int(size) < 44:
                    self.issues.append(f"[Touch Target] {filename}: Touch target size {size}px < 44px minimum (iOS: 44pt, Android: 48dp)")
        elif is_flutter:
            # Flutter IconButton has default 48x48 min touch target; check custom small GestureDetector/InkWell
            small_targets = re.findall(r'(?:GestureDetector|InkWell)\s*\([^)]*?(?:width|height):\s*([0-3]\d)', content, re.IGNORECASE | re.DOTALL)
            for size in small_targets:
                if int(size) < 44:
                    self.issues.append(f"[Touch Target] {filename}: Interactive touch target size {size}px < 44px minimum (iOS: 44pt, Android: 48dp)")
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'ios', 'android', '.idea', '.dart_tool'}]
            for file in files:
